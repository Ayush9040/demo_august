...
const ShippingAdd = () => {
  ...
  return (
    <>
      ...
      { isLoading ? <div className='global-loader' >Loading...</div>: <div className='ship_main_div'>
          ...
          <div className='ship_left_div'>
            ...
            {prevAdd.length>0 && prevAdd.map( (item,i)=><div key={i} style={ item._id === addresId ? { boxShadow:'0px 0px 20px 0px rgba(46,92,230,1)' }:{}}  onClick={()=>{ usePrevAddress(item) }} className='optional_add'>
              <div className='opt_add1'>
                ...
                <div className='opt_house'>{ item?.city } - { item?.pincode }</div>
                <div className='opt_house'>{ item?.state },{ item?.country }</div>
              </div>
            </div>)}
          </div>
          <div>
